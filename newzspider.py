...
class newzspider3(scrapy.Spider):
    ...
    def parse_details(self, response):
        ...
        with open('results.csv', 'r', newline='', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                if row[3] == url:  # Assuming the URL is the fourth column in your CSV
                    return  # Article is already in the CSV, so skip saving it again

        yield items
        with open('results.csv', 'a', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([items['title'], items['author'] if 'author' in items else '', items['story'], items['url']])
